 ...
 * goal input was `required`, which made "just give me a terminal" impossible from the only surface
...
test("the spawn dialog's goal field is optional", () => {
  const field = html.match(/<input name="goal"[^>]*>/)?.[0];
  assert.ok(field, "goal input still exists");
