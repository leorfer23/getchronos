...
function fakeCli(bootMs: number, log: string, announces = true): string {
  const file = path.join(tmp, `cli-${bootMs}-${announces}.js`);
  fs.writeFileSync(file, `
const fs = require("fs");
setTimeout(() => {
  process.stdin.setRawMode(true);
  process.stdout.write(${JSON.stringify(announces ? "\x1b[?2004h> " : "\x1b[?25l> ")});
  let got = "";
...
function run(backend: string, seed: string, bootMs: number, announces = true): Promise<string> {
  const log = path.join(tmp, `${backend}-${bootMs}.log`);
  const term = pty.spawn(process.execPath, [fakeCli(bootMs, log, announces)], { cols: 100, rows: 30, cwd: tmp, env: process.env as Record<string, string> });
  const modes = new ModeTracker();
  let lastOut = Date.now();
  let exited = false;
  term.onData((d) => { lastOut = Date.now(); modes.feed(d); });
  typeSeed({ write: (d) => term.write(d), lastOut: () => lastOut, modes, gone: () => exited, cooked: () => ttyCooked(term) }, seed, backend, { minMs: 100, quietMs: 150, maxMs: 8000, tickMs: 25 });
...
// 2026-09-26: six cursor-agent terminals a Lead opened kept only the first 1024 bytes of their seed —
// the Focus contract up to "No filler, no" — and never saw their brief. cursor never turns bracketed
// paste on, so its seed goes on silence; on a loaded machine its node boot is silent past the floor
// and the paste landed in a tty still in cooked mode. The seed must wait for the tty to go raw.
test("a silent, late CLI that never announces its input is not typed at while its tty is cooked", async () => {
  const got = await run("cursor-agent", seed, 1200, false);
  assert.equal(got, `${seed.replace(/\n/g, " ")}\r`);
});
