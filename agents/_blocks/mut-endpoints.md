...
- POST /api/tickets {workspace_id,title,repo_id?,priority?,assignee?,context?,backend?} (assignee "agent"|"human:<operator>"; context = body text) ; POST /api/tickets/:id/dispatch ; POST /api/tickets/:id/dispatch-plan ; PATCH/DELETE /api/tickets/:id ; POST /api/tickets/:id/note {text}
  · `backend:"cursor-cloud"` runs the ticket on a Cursor-hosted VM instead of a process on this Mac — offer it only in workspaces whose `backends` allow it, and only for a GitHub repo (it 400s otherwise). It keeps working while the Mac is off; say "cloud" in your reply naming it so he can tell from his phone where it is running (see VOICE).
...
- POST /api/workspaces/:id/jots {title,body?} — park a row on that client's pad ; POST /api/jots/:id/append {text} (any terminal on its own client; adds, never replaces) ; PATCH /api/jots/:id {title?,body?,status?} ; DELETE /api/jots/:id ; POST /api/jots/:id/run {backend?,model?,cwd?} (opens a terminal seeded from the row) ; POST /api/jots/:id/follow-up {at: "+2d"|"tomorrow 9:00"|ISO|null, check?} (schedule/cancel an agent follow-up) ; POST /api/jots/:id/follow-up/now (fire it now) ; POST /api/jots/:id/resolve {note?} (append the reason, mark done). POST /api/workspaces/:id/jots also takes follow_up_at + follow_up_check. Prefer the `mc pad` CLI — it resolves id prefixes and appends without clobbering.
